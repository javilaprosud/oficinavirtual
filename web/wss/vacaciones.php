...
/*
  ...
  FROM [procesadorabd].[dbo].[DetVacaciones] WHERE EmplRut = '  ".$rut."' ORDER BY DetVaAno DESC " );
	...
	FROM [procesadorabd].[dbo].[DetVacaciones] WHERE ( ltrim(rtrim(EmplRut)) = '".$rut."' OR ltrim(rtrim(EmplRut)) = '".$rut."' ) AND [DetVaFechaFin] <= TRY_PARSE('".$hasta."' AS DATETIME USING 'en-gb') AND [DetVarFechaIni] >= TRY_PARSE('".$desde."' AS DATETIME USING 'en-gb') AND [EmplCodigo] != 0 " );
		...
		,[DetVaDiasNOHabiles]
		,[DetVaDiasProgresivos]
	FROM [procesadorabd].[dbo].[DetVacaciones] WHERE EmplRut = '  ".$rut."' OR EmplRut = ' ".$rut."' AND DATEDIFF(year,[DetVaFechaFin], GETDATE()) <= 0 AND [EmplCodigo] != 0 " );
	*/
	 ...
	 $result=$db->query("SELECT TOP (1000) 
		...
		,[DetVaDiasNOHabiles]
		,[DetVaDiasProgresivos]
	FROM [procesadorabd].[dbo].[DetVacaciones] WHERE ( ltrim(rtrim(EmplRut)) = '".$rut."' OR ltrim(rtrim(EmplRut)) = '".$rut."' ) AND [DetVaAno] = ".date("Y")." AND [EmplCodigo] != 0 " );
    ...
    $result=$db->query("SELECT TOP (1000) 
		...
		,[DetVaDiasNOHabiles]
		,[DetVaDiasProgresivos]
	FROM [procesadorabd].[dbo].[DetVacaciones] WHERE  EmplRut = '  ".$rut."' OR EmplRut = ' ".$rut."' 
...
//      ,'VACACIONES PROPORCIONALES' as DetVaObs	
  ...
  FROM [procesadorabd].[dbo].[DETALLE_VACACIONES_RESUMEN_WEB] WHERE   Rut = '  ".$rut."' OR  Rut = ' ".$rut."' 
  and Empresa='1' and ((Anio=".date("Y")." 
	...
	$result=$db->query("SELECT TOP (1000)
      ...
      ,[DiasRestantes]
      ,[DiasAcumulados]
  FROM [procesadorabd].[dbo].[DETALLE_VACACIONES_RESUMEN_WEB] WHERE ltrim(rtrim(Rut)) = '".$rut."' and Empresa='1' and Anio = year(getdate()) " );
...
//(PARA VACACIONES TOTALES SUMAR PRESENTE ANO + ACUMULADO) Y TOMAR RESTANTES PARA RESTANTES Y TOTAL DIAS TOMADOS ES ESO Y LO MISMO CON DIAS RESTANTES
  ...
  FROM [procesadorabd].[dbo].[DETALLE_VACACIONES_RESUMEN_WEB] WHERE ltrim(rtrim(Rut)) = '".$rut."' and Empresa='1' and Anio = year(getdate()) order by  TotalDiasTomados Desc " );
